...
## Example 1: Simple conditional procedure
```sql
...
## Example 2: Procedure with count and average
```sql
DROP PROCEDURE IF EXISTS test;
DELIMITER //
CREATE PROCEDURE test()
...
## Example 3: Common factors procedure
```sql
DROP PROCEDURE IF EXISTS test;
DELIMITER //
CREATE PROCEDURE test()
...
## Example 5: Insert category with duplicate handling
```sql
DROP PROCEDURE IF EXISTS test;
DELIMITER //
CREATE PROCEDURE test()
